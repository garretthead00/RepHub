...
class EatActivityStore {
    ...
    class func getHourlyEnergyConsumedTotal(){ //}(completion: @escaping([Double]?, Error?) -> Void) {
        ...
        let query = HKStatisticsCollectionQuery.init(quantityType: energyConsumedQuantityType, quantitySamplePredicate: nil, options: .cumulativeSum, anchorDate: anchorDate!, intervalComponents: interval)
        ...
        query.initialResultsHandler = {
            ...
            results?.enumerateStatistics(from: startDate, to: endDate!, with: {
                (result, stop) in
                print("Time: \(result.startDate), \(result.sumQuantity()?.doubleValue(for: HKUnit.largeCalorie()) ?? 0)")
               })
           }
           HKHealthStore().execute(query)
    }
